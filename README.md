...
`/map`, `/digital-twin`, `/simulation`의 맨홀/시뮬레이션 데이터는 별도 외부 백엔드를 직접 호출합니다. 기본값은 둘 다 `http://localhost:8080`이며, 필요하면 `.env.local`에서 `VITE_EXTERNAL_SENSOR_API_BASE_URL`, `VITE_SWMM_API_BASE_URL`로 분리합니다.
...
- 통합 지도(`/map`): 레이어 토글, 선택 지역 표시, 외부 센서 API 맨홀 수위 데이터 기반 하수관로 수위 반경을 표시합니다.
- 안전 경로(`/safe-route`): 현재 위치 기준 대피소를 표시하고 자동차/도보 경로를 조회합니다.
- 디지털 트윈(`/digital-twin`): 외부 센서 API 맨홀 수위 데이터를 카카오 지도 위에 수위별 마커로 표시합니다.
- 시뮬레이션(`/simulation`): VWorld WebGL/Cesium 화면에서 외부 SWMM 테스트 결과에 따른 침수 구역 시뮬레이션을 실행합니다.
