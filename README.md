...
- 홈(`/dashboard`): 실시간 위험도 요약, 주요 위험 지역 TOP 3, 지도형 요약, 이벤트 타임라인, 센서 상태를 표시합니다.
- AI 위험도 분석(`/risk-analysis`): 지역별 실시간 수집값, 1~3시간 위험도 차트, 상황별 침수 알림을 제공합니다.
- 통합 지도(`/map`): `VITE_KAKAO_MAP_KEY`로 카카오 지도 SDK를 로드하고 자치구 대표 좌표 기준의 지역별 위험도 마커를 표시합니다. 현재는 구 단위 위험도 모니터링 목적이며, 마커 위치는 실제 침수 발생 지점이나 하수관로 센서 위치를 의미하지 않습니다.
- 안전 경로(`/safe-route`): 현재 위치 기준 대피소를 표시하고 자동차/도보 경로를 조회합니다.
- 디지털 트윈(`/digital-twin`): 침수 깊이와 위험도를 시각적으로 확인하는 화면입니다.
