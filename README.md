...
- 카카오 지도 기반 지역별 위험도 마커, InfoWindow, 예측 시간대 전환, AI 상세 분석 이동
...
- 외부 8080 백엔드가 제공하는 맨홀 수위와 SWMM 테스트 API 연동 화면
...
- Maps/Simulation: react-kakao-maps-sdk, Kakao Mobility, Tmap, VWorld WebGL, Cesium
...
`/map`, `/digital-twin`, `/simulation`의 맨홀/시뮬레이션 데이터는 별도 8080 백엔드를 직접 호출합니다. 현재 코드 기준으로 맨홀 수위는 `http://192.168.0.108:8080/api/manholes`, SWMM 테스트는 `http://localhost:8080/api/test/swmm?rainfall=...`를 사용합니다.
...
```text
React SafeRoute / ShelterMapPanel
        ↓
...
- 통합 지도(`/map`): 레이어 토글, 선택 지역 표시, `192.168.0.108:8080` 맨홀 수위 데이터 기반 하수관로 수위 반경을 표시합니다.
- 안전 경로(`/safe-route`): 현재 위치 기준 대피소를 표시하고 자동차/도보 경로를 조회합니다.
- 디지털 트윈(`/digital-twin`): `192.168.0.108:8080` 맨홀 수위 데이터를 카카오 지도 위에 수위별 마커로 표시합니다.
- 시뮬레이션(`/simulation`): VWorld WebGL/Cesium 화면에서 `localhost:8080` SWMM 테스트 결과에 따른 침수 구역 시뮬레이션을 실행합니다.
- 경보 관리(`/alerts`)와 보고서(`/reports`): 경보 이력과 보고서 대기열을 확인합니다.

대시보드와 지역별 위험도 지도는 `useDashboardStore().selectedRegion`을 공유합니다. 지도 마커 또는 InfoWindow에서 지역을 선택하면 AI 위험도 패널, 차트, 상세 분석 진입 지역이 같은 값으로 동기화됩니다.
