...
if shelters.get("success") is False:
    print(f"대피소 API 응답 오류: {shelters.get('message', '알 수 없는 오류')}")
else:
    for s in shelters.get("shelters", []):
...
if forecast.get("success") is False:
    print(f"기상청 API 응답 오류: {forecast.get('message', '알 수 없는 오류')}")
else:
