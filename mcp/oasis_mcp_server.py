...
# 요구하는 경우 .env에서 MCP_TRANSPORT=stdio로 되돌릴 수 있다.
...
if MCP_TRANSPORT not in {"stdio", "sse", "streamable-http"}:
    raise ValueError(
        "MCP_TRANSPORT는 stdio, sse, streamable-http 중 하나여야 합니다."
    )
...
"""
    try:
        ...
        # API 키, 네트워크 또는 사용량 제한 문제가 있어도 보고서 생성 자체는
        # 중단하지 않고 센서 데이터를 이용한 요약을 제공한다.
        summary = (
            f"{status.get('location_name', '대상 지역')}의 침수 위험 단계는 "
            f"{status.get('risk_level', '확인 필요')}입니다.\n"
            f"현재 수위 {status.get('water_level', '-')}%, 시간당 강수량 "
            f"{status.get('rainfall', '-')}mm가 관측되었습니다.\n"
            "현장 안전조치와 수위 변화를 지속적으로 확인하십시오."
        )
...
# 보고서 UI가 사용할 REST 호환 경로도 같은 FastMCP 서버에 등록한다.
from mcp_http_routes import register_http_routes

register_http_routes(mcp, globals())


# ── 진입점 ────────────────────────────────────────────
if __name__ == "__main__":
    if MCP_TRANSPORT == "stdio":
        # stdio 프로토콜에서는 stdout 로그가 MCP 메시지를 깨뜨릴 수 있으므로
        # 별도 안내를 출력하지 않는다.
        mcp.run(transport="stdio")
    else:
        print(
            f"OASIS MCP 서버 시작: http://{MCP_HOST}:{MCP_PORT}/mcp "
